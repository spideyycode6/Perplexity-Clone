...
const Register = () => {
  ...
  const inputStyle = (name, hasError = false) => ({
    background: '#18181b',
    border: `1px solid ${hasError ? '#f87171' : focused === name ? TEAL : '#3f3f46'}`,
    boxShadow: focused === name ? `0 0 0 3px ${hasError ? '#f8717120' : TEAL + '22'}` : 'none',
    ...
    <div className="min-h-screen flex" style={{ background: '#0c0c0f', fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <div className="hidden lg:flex lg:w-[52%] relative flex-col justify-between p-14 overflow-hidden"
        ...
        <div className="relative z-10 space-y-5">
          ...
          <h2 className="text-4xl font-bold text-white leading-snug" style={{ fontFamily: "'Syne', sans-serif" }}>
            ...
            <span style={{ color: TEAL }}>Your curiosity has no limits.</span>
          </h2>
          <p className="text-sm text-zinc-500 leading-relaxed max-w-xs">
            Join millions of users getting smarter answers, faster. No fluff, just the truth.
          </p>
        </div>
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-6 py-14 sm:px-12">
        ...
        <div className="anim-fade-up w-full max-w-[420px]">
          ...
          {/* Heading */}
          <div className="mb-8">
            <h1 className="text-2xl font-bold text-white" style={{ fontFamily: "'Syne', sans-serif" }}>Create an account</h1>
            <p className="text-zinc-500 text-sm mt-1.5">Fill in the details below to get started</p>
          </div>
          ...
          <div className="flex gap-3 mb-6">
            {[
              ...
              <button key={label} type="button"
                className="flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl text-sm font-medium text-zinc-300 transition-all duration-150 hover:text-white active:scale-[0.98]"
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <Field label="Password">
                <div className="relative">
                  <input
                    type={show.password ? 'text' : 'password'} name="password" id="password"
              ...
              <Field label="Confirm">
                <div className="relative">
                  <input
                    type={show.confirm ? 'text' : 'password'} name="confirmPassword" id="confirmPassword"
                    value={formData.confirmPassword} onChange={handleChange}
                    placeholder="••••••••" required autoComplete="new-password"
                    onFocus={() => setFocused('confirmPassword')} onBlur={() => setFocused('')}
                    className="w-full rounded-xl px-4 py-2.5 pr-11 text-sm placeholder-zinc-600"
                    style={inputStyle('confirmPassword', !!error)}
                  />
                  <button type="button" tabIndex={-1}
          ...
          <p className="mt-8 text-center text-sm text-zinc-600">
            ...
            <Link to="/login" className="font-medium transition-colors" style={{ color: TEAL }}>
              Sign in
            </Link>
          </p>
        </div>
